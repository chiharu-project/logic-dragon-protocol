...
| `can_choose` | `list[card]` | 在`object`是`hand_card`时使用，代表哪些手牌可选择 |
...
## card_effect/status_effect 卡牌持有效果/状态效果触发

| 字段名 | 数据类型 | 说明 |
| ----- | ------- | ---- |
| `card`/`status` | `ProtocolData` | 所触发的卡牌/状态 |
| `time` | str | 触发时机，与其他信息一起用于判断特有效果，可有可无，如只有一个触发时机则可不给。 |
| `any` | Any | 其他信息，见[卡牌/状态特有效果一览](#卡牌/状态特有效果一览) |
...
| `data` | string | 卡牌包含的额外数据，用以确定是否是同一张 |
